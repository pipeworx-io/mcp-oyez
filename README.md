...
`tools/list` at `https://gateway.pipeworx.io/oyez/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/oyez_search_cases \
  -H 'Content-Type: application/json' \
  -d '{"name":"Harvard","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/oyez_search_cases`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
